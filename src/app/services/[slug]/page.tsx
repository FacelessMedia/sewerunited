...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!service) return {};
  return {
    title: `${service.name} in Chicago | ${BUSINESS.name}`,
    description: `${service.description} Serving Chicagoland & suburbs. Call ${BUSINESS.phone} for 24/7 service.`,
  };
}
    ...
    <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {otherServices.map((s) => (
              <Link
                ...
                className="flex items-center gap-4 p-5 rounded-xl bg-gray-50 hover:bg-primary/5 border border-gray-100 hover:border-primary/20 transition-all group"
              >
      ...
      <section className="py-16 gradient-dark text-white">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-white/70 mb-8">Call us now for fast, professional service across Chicagoland.</p>
          <a
            href={BUSINESS.phoneHref}
